...
def multiplosCuatro():
    while True:
        entrada = input('Ingrese múltiplos de 4 para agregar a la lista: \n(0 para finalizar): ')
        try:
            num = int(entrada)
            if num == 0:
                break
            if num % 4 != 0:
                print('Error: Ingrese un múltiplo de 4.')
            else:
                agregar_una_vez(Lista_multiplos4, num)
        except ValueError:
            print('Ingrese un número válido.')
    print(f'La lista de los múltiplos de 4 es: {Lista_multiplos4}')
    print()
